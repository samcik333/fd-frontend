...
const Filters: React.FC<FiltersProps> = ({ onFilterChange }) => {
    ...
    return (
        <div className={`${filter.filter_panel} p-3`}>
            ...
            <input type="text" className="form-control mb-3" name="name" value={filters.name} onChange={handleInputChange} placeholder="Search by name" />
            <select className="form-select mb-3" name="format" value={filters.format} onChange={handleInputChange}>
                <option value="">Select format</option>
                {["Group", "Play-off", "Group+Play-off"].map((format, index) => (
                    <option key={index} value={format}>{format}</option>
                ))}
            </select>
            <input type="text" className="form-control mb-3" name="location" value={filters.location} onChange={handleInputChange} placeholder="Search by location" />
            ...
            <select className="form-select mb-3" name="sortBy" value={filters.sortBy} onChange={handleInputChange}>
                <option value="">Sort by</option>
                {["A-Z", "Z-A", "StartDate", "EndDate", "Type", "Status"].map((sortBy, index) => (
