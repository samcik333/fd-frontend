...
const Players: React.FC<{ teamId: number, teamName: string }> = ({ teamId, teamName }) => {
    ...
    const fetchPlayers = async () => {
        try {
            const response = await fetch(`http://localhost:3000/tournaments/teams/${teamId}/players`, { credentials: 'include' })
            if (response.ok) {
